...
// Default file names
const char default_kernel[] = "kernel";
...
// Block device
uint32_t blk_size;
uint32_t blk_transfer_size;
uint32_t blk_offs;
uint32_t blk_ram_ptr;
int32_t blk_err;
...
/*
	Snapshot file structure:
		- main memory
		- CPU core
	maybe it should contain main memory size too and maybe even the block device status?
*/

int SaveSnapshot(const char *filename, struct MiniRV32IMAState *core, uint8_t *ram_image)
{
	FILE *f;
	f = fopen(filename, "wb+");
	if (!f || ferror(f))
	...
	size_t written = fwrite(ram_image, 1, ram_amt, f);
	if (written != ram_amt)
		return -2;

	written = fwrite(core, 1, sizeof(struct MiniRV32IMAState), f);
	if (written != sizeof(struct MiniRV32IMAState))
		return -2;
...
int main(int argc, char **argv)
{
	...
	for (i = 1; i < argc; i++)
	{
		...
		do
		{
			if (param[0] == '-' || param_continue)
			{
				...
				case 'm':
					if (++i < argc)
						ram_amt = 1024 * 1024 * SimpleReadNumberInt(argv[i], ram_amt);
					break;
				case 'c':
					if (++i < argc)
	...
	uint8_t *image = ram_image;

	if (blk_file_name && strcmp(blk_file_name, "disable"))
	{
		blk_file = fopen(blk_file_name, "rb+");
		if (!blk_file || ferror(blk_file))
		{
			fprintf(stderr, "Error: \"%s\" not found\n", blk_file_name);
			return -5;
		}
		fseek(blk_file, 0, SEEK_END);
		blk_size = ftell(blk_file);
		fseek(blk_file, 0, SEEK_SET);
	}
...
restart:
{
	...
	// Boot from snapshot if no image filename is provided
	if (snapshot_file_name && !image_file_name)
	{
		FILE *f;
		f = fopen(snapshot_file_name, "rb");
		if (!f || ferror(f))
		{
			fprintf(stderr, "Error: \"%s\" not found\n", snapshot_file_name);
			return -5;
		...
		fread(ram_image, 1, ram_amt, f);
		fread(core, 1, sizeof(struct MiniRV32IMAState), f);
...
static void CtrlC(int sig)
{
	...
	if (image_file_name && snapshot_file_name)
	{
		if (SaveSnapshot(snapshot_file_name, core, ram_image))
			printf("Error saving snapshot!\n");
		else
...
static void HandleOtherCSRWrite(uint8_t *image, uint16_t csrno, uint32_t value)
{
	...
	{
		blk_offs = value;
		blk_err = fseek(blk_file, blk_offs, SEEK_SET);
	...
	else if (csrno == 0x154)
	{
		...
		while (nblocks--)
		{
			if (value)
			{
				...
				fwrite(blk_buf, 1, 512, blk_file);
			}
			else
			{
				fread(blk_buf, 1, 512, blk_file);
				for (int i = 0; i < 512 / 4; i++)
				{
